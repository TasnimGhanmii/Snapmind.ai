import React from 'react'

function Hero() {
  return (
    <div className='px-4 sm:px-20 xl:px-32 relative inline-flex flex-col w-full justify-center bg-[url(/gradientBackground.png)] bg-no-repeat bg-cover   min-h-screen'>
        <div className='text-center mb-6'>
            <h1 className='text-3xl sm:text-5xl md:text-6xl 2xl:text-7xl font-semibold mx-auto leading-[1.2]'>Create Amazing Content <br /><span className='text-[#6D04DE]'> with AI Tools</span></h1>
            <p className='mt-4 max-w-xs sm:max-w-lg 2xl:max-w-xl m-auto max-sm:text-xs text-gray-600'>
                Transform your content creation with our suite of premium AI Tools.
                Write articles, generate images, & enhance your workflow.
            </p>
        </div>

        <div className='flex  flex-wrap justify-center gap-4 text-sm max-sm:text-xs'>
            <button className='cursor-pointer bg-[#6D04DE] text-white px-10 py-3 rounded-lg hover:scale-102 active:scale-95 transition'>Start Creating Now!</button>
            <button className='cursor-pointer bg-white px-10 py-3 rounded-lg border border-gray-300 hover:scale-102 active:scale-95 transition'>Watch Demo</button>
        </div>
    </div>
  )
}

export default Hero